...
[role='command']
```
kubectl port-forward svc/system 9453
```

Visit the microservice at https://localhost:9453/system/properties.
endif::[]
...
```bash
curl -k -s https://localhost:9453/system/properties | jq
```
endif::[]
...
[role='no_copy']
```
Liveness:   http-get http://:9090/health/live delay=30s timeout=10s period=2s #success=1 #failure=12
Readiness:  http-get http://:9090/health/ready delay=30s timeout=10s period=2s #success=1 #failure=12
Startup:    http-get http://:9090/health/started delay=30s timeout=10s period=2s #success=1 #failure=12
```
